<template>
  <div class="box-container">
    <div v-if="num === 0" class="w-100 text-center fs-4 fw-bold">Sorry...沒有相關商品</div>
    <template v-else>
      <i
        v-if="x < 0"
        class="bi bi-arrow-left-square-fill icons-sm i-sm-l text-brown"
        @click="moveList(true)"
      ></i>
      ...
      <i
        v-if="x > step * (showNum - num)"
        class="bi bi-arrow-right-square-fill icons-sm i-sm-r text-brown"
      ...
      try {
        ...
        if (this.widthSize >= 1200) {
          ...
          this.num > this.showNum
            ? (boxWidth = (boxInsideWidth / this.num) * 6)
            : (boxWidth = this.step * this.num)
        } else if (this.widthSize <= 1199 && this.widthSize >= 992) {
          this.showNum = 5
          this.num > this.showNum
            ? (boxWidth = (boxInsideWidth / this.num) * 5)
            : (boxWidth = this.step * this.num)
        } else if (this.widthSize <= 991 && this.widthSize >= 768) {
          this.showNum = 3
          this.num > this.showNum
            ? (boxWidth = (boxInsideWidth / this.num) * 3)
            : (boxWidth = this.step * this.num)
        } else if (this.widthSize <= 767) {
          this.showNum = 1
          this.num > this.showNum
            ? (boxWidth = boxInsideWidth / this.num)
            : (boxWidth = this.step * this.num)
        }
    ...
    widthSize(newWidth, oldWidth) {
      if (oldWidth) {
        ...
        if (newWidth >= 1200 && oldWidth < 1200) {
          ...
          this.showNum = 6
          this.num > this.showNum
            ? (boxWidth = (box.clientWidth / this.num) * 6)
            : (boxWidth = this.step * this.num)
          ...
          this.showNum = 5
          this.num > this.showNum
            ? (boxWidth = (box.clientWidth / this.num) * 5)
            : (boxWidth = this.step * this.num)
          ...
          this.showNum = 3
          this.num > this.showNum
            ? (boxWidth = (box.clientWidth / this.num) * 3)
            : (boxWidth = this.step * this.num)
