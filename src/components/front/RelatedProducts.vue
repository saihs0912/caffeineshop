<template>
  <div class="box-container">
    ...
    <template v-else>
      <i v-if="x < 0" class="bi bi-arrow-left-square-fill icons-sm i-sm-l text-brown" @click="moveList(true)"></i>
      ...
      <i v-if="x > step*(showNum-num)" class="bi bi-arrow-right-square-fill icons-sm i-sm-r text-brown" @click="moveList(false)"></i>
    </template>
  </div>
</template>
    ...
    widthSize (newWidth, oldWidth) {
      if (oldWidth) {
        ...
        if (newWidth >= 1200 && oldWidth < 1200) {
          ...
          this.showNum = 6
          this.num > this.showNum ? boxWidth = (box.clientWidth / this.num) * 6 : boxWidth = this.step * this.num
